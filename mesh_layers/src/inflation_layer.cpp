    ...
    std::set<lvr2::VertexHandle>& removed_lethal)
{
  ...
  lethalCostInflation(
      ...
      config.lethal_value == -1 ? std::numeric_limits<float>::infinity() : config.lethal_value);
}
...
void InflationLayer::reconfigureCallback(mesh_layers::InflationLayerConfig& cfg, uint32_t level)
{
  ...
  lethalCostInflation(
      ...
      cfg.inscribed_radius,
      cfg.inscribed_value,
      cfg.lethal_value == -1 ? std::numeric_limits<float>::infinity() : cfg.lethal_value);
