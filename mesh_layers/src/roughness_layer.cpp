...
bool RoughnessLayer::readLayer()
{
  ...
  if(roughness_opt){
    ...
    {
      if(roughness[vH] > config.threshold) lethal_vertices.insert(vH);
    }
    return true;
  }
  else{
...
bool RoughnessLayer::computeLayer()
{
  ...
  auto face_normals_opt =
      mesh_io_ptr->getDenseAttributeMap<lvr2::DenseFaceMap<mesh_map::Normal>>("face_normals");
  ...
  auto vertex_normals_opt =
      mesh_io_ptr->getDenseAttributeMap<lvr2::DenseVertexMap<mesh_map::Normal>>("vertex_normals");
...
bool RoughnessLayer::initialize(const std::string& name)
{
  ...
  reconfigure_server_ptr = boost::shared_ptr<dynamic_reconfigure::Server<mesh_layers::RoughnessLayerConfig> > (
      new dynamic_reconfigure::Server<mesh_layers::RoughnessLayerConfig>(private_nh));

  config_callback = boost::bind(&RoughnessLayer::reconfigureCallback, this, _1, _2);
